...
if [ -z "$1" ]; then
    echo "Użycie: ~/ros_scripts/process_real_slam.sh nazwa_baga"
    echo "Przykład: ~/ros_scripts/process_real_slam.sh rosbag2_real_slam_01"
    exit 1
fi

BAG_NAME="$1"
BAG_PATH="$HOME/$BAG_NAME"
RESULT_DIR="$HOME/ros_scripts/wyniki/slam_toolbox_real/$BAG_NAME"
RPI_IP="192.168.0.57"
...
echo "=== Rozpakowanie CSV i wykresu ==="
python3 ~/ros_scripts/extract_one_bag_slam_toolbox_real.py "$BAG_PATH"
