...
WS="$HOME/ros2_ws"
RESULT_BASE="$HOME/ros_scripts/wyniki/cartographer"
RPI_IP="192.168.0.57"
...
# Szuka najnowszego rosbaga po czasie modyfikacji, a nie po nazwie.
LATEST_BAG=$(find "$WS" -maxdepth 1 -type d -name "rosbag2_*" -printf "%T@ %p\n" | sort -n | tail -n 1 | cut -d' ' -f2-)
...
echo "Najnowszy bag: $LATEST_BAG"
echo "Folder wyników: $RESULT_DIR"

mkdir -p "$RESULT_DIR"

echo "=== Sprawdzenie zawartości rosbaga ==="
ros2 bag info "$LATEST_BAG" || true

echo "=== Ekstrakcja trajektorii i wykresów Cartographera ==="
python3 "$HOME/ros_scripts/extract_one_bag_cartographer.py" "$LATEST_BAG"

echo "=== Ekstrakcja OSTATNIEJ mapy /map z rosbaga ==="

python3 - "$LATEST_BAG" "$RESULT_DIR" <<'PY'
import sys
import os
import math
import yaml
...
def save_pgm(filename, grid, width, height):
    ...
    # ROS OccupancyGrid ma początek mapy na dole,
    # a obraz PGM zapisujemy od góry, więc odwracamy oś Y.
    data = np.flipud(data)

    img = np.zeros((height, width), dtype=np.uint8)

    # Standard map_server:
    # wolne = jasne, zajęte = czarne, nieznane = szare
    img[data == -1] = 205
    img[data >= 65] = 0
...
if not os.path.isdir(bag_path):
    print(f"Nie znaleziono rosbaga: {bag_path}")
    sys.exit(1)
...
print("Zapisano ostatnią mapę z rosbaga:")
print(f" - {pgm_path}")
print(f" - {yaml_path}")
PY
